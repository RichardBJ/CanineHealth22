...
binary <- as.data.frame(cbind(as.numeric(ml$C_Gender)-1, ml$D_Sex, ml$D_Neuter,ml$Meds,  ml$D_Diet_Vegan, ml$D_Diet_Raw,ml$C_Diet_Vegan, as.factor(ml$Animal_Career_BINARY),  as.factor(ml$Therapeutic_Food), ml$Any_Health_Problem))
...
names(cdata)[1:5] <- c("UK", "Other European","North American", "Oceania", "Location Other")
names(cdata)[8:9] <- c("Dog Age","Owner Age" )
names(cdata)[11:14] <- c("Vet Visits", "Owner Gender", "Dog Sex", "Neuter Status")
names(cdata)[16:20] <- c("Dog Vegan Diet", "Dog Raw Diet", "Owner Vegan Diet", "Animal Career",
                         "Therapeutic Diet")
...
if (WRITE){
  write.csv(corMatrix_p, "corr-p-values.csv")
  write.csv(round(corMatrix,2), "corr-values.csv")
  write.csv(logmatrix,"corr-logp-values.csv")
  }
...
corrplot::corrplot(cor.Mat, p.mat = tmp, type="lower", order="hclust", hclust.method="ward.D2",insig = "blank", tl.col="black", sig.level = bht, tl.srt=45,tl.cex=cex/2,na.label=" ",cl.cex = cex/2)

#Then save figure
tiff("HealthBinaryCorPlot.tif",  
  width     = 3.25,
  height    = 3.25,
  units     = "in",
  res       = 300,
  pointsize = 4)
  par(bg=NA)
corrplot::corrplot(cor.Mat, p.mat = tmp, type="lower", order="hclust", hclust.method="ward.D2",insig = "blank", tl.col="black", sig.level = bht, tl.srt=45,tl.cex=cex,na.label=" ",cl.cex = cex)
...
prediction <- as.numeric(pred[,2] > 0.60)
roc(trainY, pred[,2])
confusionMatrix(as.factor(prediction),as.factor(trainY))
######

```
# Draw ROC ALL Variables included, Any health issue binary
```{r roc1_ALL}
...
importance_matrix$Feature <-c("Vet Visits","Meds","Dog Age","Owner Age","Education","Dog Vegan Diet","Size","Income",
             "Dog Raw Diet","UK", "Dog Sex","Owner Gender","Other European","Animal Career",
             "Owner Vegan Diet","Oceania","Neuter Status", "Location Other","North American")

tiff("xgAnybwIMPsimpleFull.tif")
xgb.plot.importance(importance_matrix = importance_matrix)
dev.off()
...
# Simple XGBoost on reduced variable set, still Any Health Issue binaru
```{r train_XG_reducedecho=TRUE, include=TRUE, warning=FALSE, errors=FALSE, warning=FALSE, message=FALSE,results='hide'}
rm(cv_res)
rm(bstSparse)

factors <- dplyr::select(ml, Location)
factors <- mltools::one_hot(as.data.table(factors),dropUnusedLevels = TRUE)
...
#numeric <- dplyr::select(ml,D_Age)
binary <- as.data.frame(cbind(as.numeric(ml$C_Gender)-1, ml$D_Sex, ml$D_Neuter,ml$D_Diet_Vegan, ml$C_Diet_Vegan, as.factor(ml$Animal_Career_BINARY),   ml$Any_Health_Problem))
...
print(importance_matrix)
xgb.plot.importance(importance_matrix = importance_matrix)
importance_matrix$Feature <-c("Dog Age","Owner Age","Education","Owner Gender", "Size","Dog Sex","UK","Dog Vegan Diet",
                              "Location Other",  "Oceania", "Income", "Other European", "Neuter Status",
                              "Owner Vegan Diet","Animal Career", "North American")
xgb.plot.importance(importance_matrix = importance_matrix)
m_imp <-reshape::melt(importance_matrix,id_vars=Feature)
names(m_imp) <- c("Feature", "XGBoost Parameter", "Value")
ggplot(m_imp,aes(x=reorder(Feature,-Value),y=Value,fill=`XGBoost Parameter`))+geom_bar(position="stack", stat="identity")+xlab("Feature") + ylab("Value (Au)")+
